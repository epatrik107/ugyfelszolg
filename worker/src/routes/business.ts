...
export async function sendBusinessAccessLinkRoute(c: Context<{ Bindings: Env }>) {
  ...
  if (!parsed.success) {
    return errorJson(c, "INVALID_INPUT", "Hibás kérés.", 400);
  }
...
export async function exchangeBusinessMagicLinkRoute(c: Context<{ Bindings: Env }>) {
  ...
  if (!parsed.success) {
    return errorJson(c, "INVALID_INPUT", "Hibás token.", 400);
  }
  ...
  if (
    ...
    return errorJson(c, "UNAUTHORIZED", "A link érvénytelen vagy lejárt.", 401);
  }

  if (!(await consumeMagicLink(c.env, magicLink.id))) {
    return errorJson(c, "UNAUTHORIZED", "A link már felhasználásra került.", 401);
  }
...
export async function getBusinessSessionRoute(c: Context<{ Bindings: Env }>) {
  ...
  if (!session) {
    return errorJson(c, "UNAUTHORIZED", "Nincs érvényes céges munkamenet.", 401);
  }
  ...
  if (!subscription) {
    return errorJson(c, "NOT_FOUND", "Az előfizetés nem található.", 404);
  }
...
export async function createBusinessOrderRoute(c: Context<{ Bindings: Env }>) {
  ...
  if (!session || !["active", "trialing"].includes(session.status)) {
    return errorJson(c, "UNAUTHORIZED", "Nincs aktív céges előfizetés.", 401);
  }
  ...
  if (!hasAvailableQuota(usage) || !(await reserveQuota(c.env, usage))) {
    return errorJson(c, "CONFLICT", "A havi levélkeret elfogyott.", 409);
  }
  ...
  return okJson(c, { publicId, resultToken });
}
...
export async function createBusinessPortalSessionRoute(c: Context<{ Bindings: Env }>) {
  ...
  const session = await getSessionFromAuthHeader(c);
  if (!session) {
    return errorJson(c, "UNAUTHORIZED", "Nincs érvényes munkamenet.", 401);
  }
  ...
  return okJson(c, { url: portal.url });
}
